...
def _get_cuda_arch_flags(cflags: Optional[list[str]] = None) -> list[str]:
    ...
    def _arch_sort_key(arch_str: str) -> tuple[int, int, int]:
        match = re.match(r'^(\d+)\.(\d+)(a?)$', arch_str)
        if not match:
            return (0, 0, 0)
        major, minor, suffix = match.groups()
        return (int(major), int(minor), 1 if suffix == 'a' else 0)

    fallback_base_arch = max(supported_arches, key=_arch_sort_key)
    ...
    for arch in arch_list:
        if arch not in valid_arch_strings:
            fallback_arch = (f'{fallback_base_arch}+PTX'
                             if arch.endswith('+PTX') else fallback_base_arch)
            warnings.warn(
                f"Unknown CUDA arch ({arch}); falling back to {fallback_arch}.")
            arch = fallback_arch
